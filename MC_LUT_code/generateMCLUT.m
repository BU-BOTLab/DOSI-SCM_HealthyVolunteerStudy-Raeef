function [LUT] = generateMCLUT(allMua,allMus, rho, freqs)
...
%Otherwise, make the LUT
    ...
    [tpsf,tvec, ~,~] = getTPSF_LUT(basedir,fname,secPerSamp,endTime,n,NA,unitInmm,1,transFlag);
    ...
    for i = 1:size(X,1)
        parfor j = 1:size(X,2) %Use a parallel for loop for speed
            rawDat = forwardMC([X(i,j),Y(i,j)], freqs, tpsf, tvec, muspVec, n,reImFlag,0);
            realPart = rawDat(1,:);
            imagPart = rawDat(2,:);
            rp(i,j,:) = realPart;
            ip(i,j,:) = imagPart;
